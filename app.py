...
def run_simulation(num_tasks, num_employees, num_episodes, container, log_container):
    ...
    # Train RL models
    progress_text.text(
        f"Training RL models for {config.refinement.num_episodes} episodes..."
    )
    ...
    for episode in range(config.refinement.num_episodes):
        ...
        if episode % max(1, config.refinement.num_episodes // 10) == 0:
            ...
            progress_text.text(
                f"Training episodes: {episode + 1}/{config.refinement.num_episodes}"
            )
...
def main():
    ...
    if st.sidebar.button("Run Simulation", type="primary"):
        ...
        results = run_simulation(
            num_tasks, num_employees, num_episodes, progress_container, log_container
        )
        ...
        if results:
            ...
            with tab3:
                ...
                display_image_with_download(
                    "Q-learning Before Refinement", "plots/gantt_q_before.png", col1
                )
                display_image_with_download(
                    "SARSA Before Refinement", "plots/gantt_sarsa_before.png", col2
                )

                col1, col2 = st.columns(2)
                display_image_with_download(
                    "Greedy Assignment", "plots/gantt_greedy.png", col1
                )
                display_image_with_download(
                    "Q-learning After Refinement", "plots/gantt_q_after.png", col2
                )

                display_image_with_download(
                    "SARSA After Refinement", "plots/gantt_sarsa_after.png", st
                )
